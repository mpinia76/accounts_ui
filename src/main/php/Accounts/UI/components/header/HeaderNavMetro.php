		...
		foreach ($menuGroups as $menuGroup) {
			...
			if($size>0){
				...
				foreach ($menuGroup->getMenuOptions() as $menuOption) {
                    ...
                    $size2 = count($menuOption->getSubMenuOptions());
                    if ($size2 > 0) {


                        foreach ($menuOption->getSubMenuOptions() as $subMenuOption) {
                            //print_r($subMenuOption);

                            $xtpl->assign("liCss", "has-sub");
                            $xtpl->assign("label", $subMenuOption->getLabel());
                            $xtpl->assign("onclick", $subMenuOption->getOnclick());
                            $currentPage='';
                            if ($subMenuOption->getSelected()){
                                $currentPage='page';
                            }
                            $xtpl->assign("page", $currentPage);
                            $img = $subMenuOption->getImageSource();
                            if (!empty($img)) {
                                $xtpl->assign("src", $img);
                                $xtpl->parse("main.$blockName.menuGroup.options.menuOption.subOptions.subMenuOption.image");
                            } else {
                                $icon = $subMenuOption->getIconClass();
                                if (!empty($icon)) {
                                    $xtpl->assign("iconClass", $icon);
                                    $xtpl->parse("main.$blockName.menuGroup.options.menuOption.subOptions.subMenuOption.icon");
                                }
                            }
                            $xtpl->parse("main.$blockName.menuGroup.options.menuOption.subOptions.subMenuOption");


                        }

                        $xtpl->parse("main.$blockName.menuGroup.options.menuOption.subOptions");
                    ...
                    $selectedPage='cd-side__item cd-side__item--has-children js-cd-item--has-children';
                    if ($menuOption->getSelected()){
                        $selectedPage='cd-side__item cd-side__item--has-children cd-side__item--selected js-cd-item--has-children';
                    }
                    $xtpl->assign("itemClass", $selectedPage);
